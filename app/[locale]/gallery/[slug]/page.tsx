...
export async function generateMetadata({
  ...
  const destination = getDestination(slug, locale)
...
export default async function JourneyPage({
  ...
  const next = getNextDestination(slug, locale)

  return <JourneyPageContent destination={destination} next={next} locale={locale} />
}
...
function JourneyPageContent({
  ...
  next: ReturnType<typeof getNextDestination> & {}
  locale: string
}) {
    ...
    <article>
      ...
      <header className="relative flex min-h-[480px] items-end" style={{ height: "80vh" }}>
        ...
        <div className="relative mx-auto w-full max-w-4xl px-5 pb-12 sm:px-8 sm:pb-16">
          ...
          {destination.nameAr && !isAr ? (
            <>
              <h1
                className="mt-3 text-balance font-display text-5xl leading-[1.05] text-warm-white sm:text-6xl lg:text-7xl"
                lang="ar"
                dir="rtl"
              ...
              <p className="mt-2 font-display text-2xl text-warm-white/70">
                {destination.name}
              </p>
            </>
          ) : (
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-5 text-center">
          ...
          {next.nameAr && !isAr ? (
            <>
              <h2
                className="mt-3 font-display text-4xl text-warm-white sm:text-6xl"
                lang="ar"
                dir="rtl"
                style={{ fontFamily: 'var(--font-arabic), serif' }}
              >
              ...
              <p className="mt-1 font-display text-xl text-warm-white/70">
                {next.name}
              </p>
            </>
          ) : (
